...
t_lista *cria_lista_times()
{
    ...
    /* 
    define a localização da via de comunicação relacionado a codificação dos mesmos, 
    facilita os caracteres multibytes
    */
    ...
    { // loop que percorre
        ...
        if (fgets(str[i], sizeof(str[i]), arq) == NULL)
        {               // leitura da i-ésima linha com condição para terminar o loop
    ...
    for (int i = 1; i < linhas; i++)
    {
        ...
        for (n = 0, j = k; str[i][j] != '\n'; j++, n++)
        { // loop para pegar parte da string que contem apenas os atributos (numeros inteiros)
